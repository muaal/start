...
public class FiveInARow extends JFrame implements MouseListener {
    private Vector v = new Vector();    //所有的每步走棋信息
    private Vector white = new Vector(); //白方走棋信息
    private Vector black = new Vector(); //黑方走棋信息
    boolean b; //用来判断白旗还是黑棋
    private int whiteCount, blackCount; //计算悔棋步数
    private int w = 25;    //间距大小
    private int px = 100, py = 100;    //棋盘的大小
    private int pxw = px + w, pyw = py + w;
    private int width = w * 16, height = w * 16;
    private int vline = width + px;    //垂直线的长度
    private int hline = height + py; //水平线的长度
...
//					System.out.println(e.getX()+"-"+e.getY());
        ...
        if (e.getButton() == MouseEvent.BUTTON3) {    //右击悔棋的方法
